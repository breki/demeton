...
type Move =
    { Direction: MoveDirection; Count: int }
...
let private directionsToMoves
    (movesSoFar, prevDirectionMaybe, prevMoveRepeatCount)
    directionMaybe
    =
...
let private clippedStepsToMoves (isoline: ClippedIsoline) : IsolineMoves =
    ...
    let _, directions = isoline.Steps |> Seq.fold processNextStep (None, [])
