...
let allNodes<'Tree> isNode leftChild rightChild tree : VisitedNode<'Tree> seq =
    ...
    let rec visitPrivate node level path : VisitedNode<'Tree> seq =
        seq {
            ...
            | true ->
                ...
                yield!
                    visitPrivate (node |> leftChild) (level + 1) (Left :: path)
...
let treeToDot
    ...
    =
    if isNode tree then
        let builder = buildString () |> appendLine "digraph BST {"
...
let private addNodeToTreeLevels
    (treeLevels: TreeLevels<'Tree>)
    (node, depth, path)
    =
    let indexFold (indexSoFar, depth) (branch: BinaryTreeBranch) : int * int =
        let branchValue =
            match branch with
            | Left -> 0
            | Right -> 1
    ...
    let nodePathToIndex (path: BinaryTreePath) =
        ...
        | [] -> 0
        | _ ->
            let index, _ = path |> Seq.rev |> Seq.fold indexFold (0, 0)
            index
                    ...
                    let maxNodeWidthInLevel =
                        ...
                        |> Seq.fold
                            (fun maxWidth node ->
                                ...
                                max nodeWidth maxWidth)
                            0

                    max maxNodeWidthInLevel maxWidth)
                0
                    ...
                    let nodesLineText =
                        ...
                        |> Seq.fold
                            (fun line node ->
                                ...
                                if
                                    finalNodeString.Length <> spaceForEachNode
                                then
                                    invalidOp "bug: this should never happen"
                                else
                                    line + finalNodeString)
                            ""
