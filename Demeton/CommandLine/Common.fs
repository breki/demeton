...
type CommandSwitch = { Name: string; Description: string }
...
let nextArg (state: ParsingState) : string option * ParsingState =
    ...
    let consumeArg state =
        match state with
        | ParsingInProgress([], parsedParameters) -> NoMoreArgs parsedParameters
    ...
    | ParsingInProgress(args, _) -> (Some(args |> List.head), consumeArg state)
...
let invalidArgumentValue name reason =
    let message = sprintf "<%s> argument's value is invalid, %s." name reason
    ParsingFail message
...
let validateSupportedParameters args (supportedParameters: CommandParameter[]) =
...
let parseParameters
    ...
    let consumeNextCommandArg
        ...
        | None, CommandParameter.Arg { IsMandatory = false }, _ ->
            ...
            Some state
        | Some arg,
          CommandParameter.Arg { Name = argName; IsMandatory = true },
          _ when arg.StartsWith ParameterPrefix ->
            argumentValueIsMissing argName |> Some
        | Some arg, CommandParameter.Arg { IsMandatory = false }, _ when
            arg.StartsWith ParameterPrefix
            ->
            None
        | Some arg, CommandParameter.Arg { Name = argName; Parser = parser }, _ ->
            ...
            | OkValue value ->
                ...
                |> Some
            | InvalidValue reason -> invalidArgumentValue argName reason |> Some
        | _ -> invalidOp "todo: "

    let consumeNextOptionOrSwitch (arg: string option) state =
        match (arg, state) with
        | Some argParameter, _ when argParameter.StartsWith(ParameterPrefix) ->
            ...
            | Some(CommandParameter.Switch switch) ->
                state |> appendParameter (ParsedSwitch { Name = switch.Name })
        ...
        | Some argParameter, _ ->
            ParsingFail(sprintf "Unrecognized parameter '%s'." argParameter)
    ...
    match state with
    | NoMoreArgs parsedParameters -> Ok(parsedParameters |> List.rev)
