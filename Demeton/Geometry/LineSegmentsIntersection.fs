...
let findParallelLineSegmentsIntersection tolerance (p1, p2) (p3, p4) =

    if (p1 = p3 && p2 = p4) || (p1 = p4 && p2 = p3) then
        LineSegmentsIntersectionResult.CollinearOverlapping p1
    elif p1 = p3 || p2 = p3 then
        SharingOneEndpoint p3
    elif p1 = p4 || p2 = p4 then
        SharingOneEndpoint p4
    else
        let left = left tolerance (area2 p1 p2 p3)

        if left <> Collinear then
            DoNotIntersect
...
let findLineSegmentsIntersection
    ...
    if (x1 = x2 && y1 = y2) || (x3 = x4 && y3 = y4) then
        OneOrBothAreZeroLength
    else
        let denom =
        ...
        if denom |> isZero tolerance then
            ...
            match
                (determineValue01Status tolerance s),
                (determineValue01Status tolerance t)
            with
