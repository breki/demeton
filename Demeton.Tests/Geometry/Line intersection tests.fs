...
type LineIntersectionPropertyTest
    ...
    let isCollinear p1 p2 p3 = left defaultTolerance (area2 p1 p2 p3) = Collinear
    ...
    let lineSegmentIntersectionProperties minLineLength (line1, line2) =
        ...
        let ``situation matches the returned result`` =
            ...
            | _ when ((line1 |> isCollinearOverlappingWith 0. line2)
            ...
            | _ when ((line1 |> liesOnTheSameSideOf lowTolerance line2)
        ...
        if (line1 |> length < minLineLength)
           || (line2 |> length < minLineLength) then
