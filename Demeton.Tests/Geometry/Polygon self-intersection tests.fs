...
let isPolygonSelfIntersectingBrute edgesIntersectFunc polygon =
    ...
    | _ ->
        ...
        let isSelfIntersecting =
            edges
            |> Seq.exists (fun e1 -> edges |> Seq.exists (edgesIntersect e1))
...
let ``polygon properties`` (vertices: Point[]) =
    ...
    let resultBrute =
        isPolygonSelfIntersectingBrute
            (edgesIntersectDefaultFunc tolerance)
            polygon

    let result =
        isPolygonSelfIntersecting (edgesIntersectDefaultFunc tolerance) polygon
    ...
    let ``polygon self-intersection detection is correct`` =
        result = resultBrute
        |> Prop.label
            "the production function result does not match the brute function"
    ...
    |> Prop.classify
        (verticesCount > 3 && verticesCount < 20)
...
type SelfIntersectingPolygonTests(output: Xunit.Abstractions.ITestOutputHelper)
    ...
    member this.runTests genCoord =
        let gen =
            Gen.frequency[(10, genLikelyIntersectingPolygon genCoord)
