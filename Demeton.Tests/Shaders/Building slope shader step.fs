...
let isError errorDescription step =
    step = Error(
        sprintf "Error in step '%s': %s" StepNameSlopeShading errorDescription
    )
...
let ``Reports an error when horizontal color is invalid`` () =
    ...
    test
        <@ step |> isError "'hcol' parameter value error: invalid color value." @>
...
let ``Reports an error when vertical color is invalid`` () =
    ...
    test
        <@ step |> isError "'vcol' parameter value error: invalid color value." @>
