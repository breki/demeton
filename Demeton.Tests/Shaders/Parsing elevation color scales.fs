...
let insertRandomWhitespace (scaleString: string) =
    ...
    genSelectedChar
    |> Gen.map (fun index -> scaleString |> insertIntoString "  " index)

[<Fact>]
let ``Testing properties of elevation color scale parsing`` () =
    let genColor =
        Gen.frequency [ (1, ColorGen.colorWith1Alpha); (8, ColorGen.color) ]

    let genInvalidScale = Arb.generate<string> |> Gen.map InvalidScale

    let genElevation = Gen.choose (-1000, 5000) |> Gen.map (int16 >> DemHeight)
    ...
    let genCase =
        Gen.frequency
            [ (6, genInvalidScale); (6, genCaseFromScale); (1, genNoNoneColor) ]

    genCase |> Arb.fromGen |> Prop.forAll
    <| ``elevation color scale parsing properties``
