...
let ``Color premultiplication properties`` color =
    ...
    let fromPremultiplied = AlphaCompositing.toRgbaColor premultiplied alpha
    ...
    | _ ->
        ...
        |@ sprintf
            "%s <> %s"
            (Rgba8Bit.toHex fromPremultiplied)
            (Rgba8Bit.toHex color)
...
let ``Color premultiplication`` () =
    ...
    genColor |> Arb.fromGen |> Prop.forAll
    <| ``Color premultiplication properties``
...
let ``Alpha compositing over operation properties`` (source, dest) =
    ...
    let props2 =
        props
        .&. (composedA >= sourceA |> Prop.label "composed alpha >= source alpha"
...
let ``Alpha compositing over operation`` () =
    ...
    Gen.zip genColor genColor |> Arb.fromGen |> Prop.forAll
    <| ``Alpha compositing over operation properties``
    |> Check.QuickThrowOnFailure
