...
[<Fact>]
let ``Sample case 1``() =
    let bounds = { MinLon = 0.; MinLat = -2.; MaxLon = 2.; MaxLat = 0. }
...
let ``When bounds cover just a single tile inside 0,0 lon/lat, level 0``() =
    let bounds = { 
        MinLon = 0.1; MinLat = 0.1; MaxLon = 0.2; MaxLat = 0.2 }
...
let ``When bounds cover just a single tile (case 2)``() =
    let bounds = { 
        MinLon = 0.1; MinLat = -0.2; MaxLon = 0.2; MaxLat = -0.1 }
    let tiles = boundsToTiles 3600 (SrtmLevel.fromInt 0) bounds
...
let ``Correctly calculates tiles lon/lat bounds``() =
    ...
    test <@ bounds = 
                { MinLon = 0.; MinLat = -2.; MaxLon = 2.; MaxLat = 0. } @>
