...
let private calculateMinDeltaUsingBruteForce
    ...
    points |> Seq.map (srtmMinCellEnergy mapProjection.Invert) |> Seq.min
...
let ``Determines the min lon/lat delta using simulated annealing`` () =
    ...
    let minDeltaUsingSimAnn = minLonLatDelta rasterRect projection.Invert
...
let ``Calculates the SRTM level needed from the lon/lat delta``
    ...
    test
        <@ (lonLatDeltaToDemLevel tileSize actualRadDelta).Value = expectedLevel @>
