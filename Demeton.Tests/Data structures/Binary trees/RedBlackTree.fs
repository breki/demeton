...
let color (node: Tree<'T>) = if node |> isBlack then Black else Red
...
let treeToDot (tree: Tree<'T>) =
    ...
    tree |> treeToDot isNode nodeAttributes leftChild rightChild
...
module private Node =
    ...
    let rec insert
        ...
        if item < parent.Item then
            ...
            | Node leftNode ->
                ...
                match
                    leftNode |> insert log (depth + 1) item (leftChildOf parent)
                with
                | RepairParent leftNode' ->
                    match parent.Color with
                    | Black -> parent |> updateLeft leftNode' |> RepairParent
                    | Red ->
                        match grandparent |> aunt |> color with
                        | Red ->
                            parent
                ...
                | LeftRotate(Node leftNode') ->
                    log
                        depth
                        (sprintf
                            "left rotate on left node:\n%A"
                            (leftNode' |> Node |> treeToDot))

                    let parentRotatedToLeft: Tree<'T> =
                        parent |> updateRightAndColor leftNode'.Left Red
                ...
                | RightRotate(Node leftNode') ->
                    let parentRotatedToRight: Tree<'T> =
                        parent |> updateLeftAndColor leftNode.Right Red
            ...
            | Node rightNode ->
                ...
                match
                    rightNode
                    |> insert log (depth + 1) item (rightChildOf parent)
                with
                | RepairParent rightNode' ->
                    match parent.Color with
                    | Black -> parent |> updateRight rightNode' |> RepairParent
                    | Red ->
                        match grandparent |> aunt |> color with
                        | Red ->
                            parent
                ...
                | LeftRotate(Node rightNode') ->
                    let parentRotatedToLeft: Tree<'T> =
                        parent |> updateRightAndColor rightNode'.Left Red
                ...
                | RightRotate(Node rightNode') ->
                    let parentRotatedToRight: Tree<'T> =
                        parent |> updateLeftAndColor rightNode'.Left Red
                ...
                | RepaintGrandparentAndAunt rightNode' ->
                    ...
                    parent
                    |> updateLeftRightAndColor repaintedAunt rightNode' Red
                    |> RepairParent
                | _ -> invalidOp "bug: this should never happen"
    ...
    let private replaceWithSuccessor node =
        ...
        | Node right ->
            let successorNodeItem, newRightChild = right |> removeSuccessor
    ...
    let rec tryRemove item node : TryRemoveResult<'T> =
        if item = node.Item then
            ...
            | Node leftNode ->
                ...
                | Found newLeftNode -> node |> updateLeft newLeftNode |> Found
                | NotFound -> NotFound
        else
            match node.Right with
            | None -> NotFound
...
let rec items tree =
    ...
    | Node node ->
        ...
        rightItems |> Seq.append [ node.Item ] |> Seq.append leftItems

/// Returns the height of the tree.
let rec height tree =
    match tree with
    | None -> 0
    | Node node -> max (node.Left |> height) (node.Right |> height) + 1
...
let treeToAscii tree =
    ...
    tree |> treeToAscii height None isNode itemToString leftChild rightChild
