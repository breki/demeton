...
let ``Properties of merging height arrays``
    (arraysToMerge: HeightsArray list, newArrayBounds: Raster.Rect) =
    
    let findCellHeightInArrays (x, y): DemHeight =
        arraysToMerge
        |> List.tryPick (fun (ha: HeightsArray) ->
            if x >= ha.MinX && x <= ha.MaxX && y >= ha.MinY && y <= ha.MaxY then
    ...
    let merged = Dem.merge newArrayBounds arraysToMerge
    ...
    let emptyList =
        ...
        | [] ->
            ...
            |> Prop.label "merged array is None if the heights arrays list is empty"
        | _ ->
            true
            |> Prop.classify true "Non-empty heights arrays list"
    ...
    let mergedNone =
        ...
        | false ->
            true
            |> Prop.classify true "Merging width and height > 0"
            ...
            let comparison = [|
                for y in mergedArray.MinY .. mergedArray.MaxY do
                    for x in mergedArray.MinX .. mergedArray.MaxX do
                        ...
                        let mergingCellsHeight = findCellHeightInArrays (x, y)
...
let ``Merging height arrays``() =
    ...
    let genRect =
        Gen.map4
            ...
            genMinX genMinY genWidth0 genHeight0
    
    let genDemHeight =
        Gen.choose(-100, 100)
        |> Gen.map DemHeight |> Gen.optionOf
    ...
    let genHeightArray =
        Gen.map4 (fun minX minY width height ->
            let heightValues = genDemHeight |> Gen.sample 100 (width*height)
            HeightsArray
                (minX, minY, width, height,
                 HeightsArrayInitializer1D (fun i -> heightValues.[i])))
            genMinX genMinY genWidth genHeight
            
    let genListOfArrays = genHeightArray |> Gen.listOf |> Gen.resize 5
    
    Gen.zip genListOfArrays genRect
    |> Arb.fromGen
    |> Prop.forAll <| ``Properties of merging height arrays``
